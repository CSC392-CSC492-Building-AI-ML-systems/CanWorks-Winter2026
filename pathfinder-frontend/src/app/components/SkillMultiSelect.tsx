...
export function SkillMultiSelect({
  ...
  // allow override when deploying; use NEXT_PUBLIC_API_BASE env var or default to localhost
  ...
  useEffect(() => {
    const fetchFilteredSkills = async () => {
      ...
      try {
        const response = await fetch(`${API_BASE}/api/skills?q=${encodeURIComponent(query)}`);
        if (response.ok) {
          const data = await response.json();
